...
public class SVersionBoundTests
{
    ...
    public void npm_versions_projections_with_includePrerelease_false( string initial, string projected, string bound )
    {
        ...
        rNpm.IsValid.Should().BeTrue();
        rNpm.Result.ToNpmString().Should().Be( projected );
    ...
    [Description( "includePrerelease is not really used in the npm ecosystem." )]
    [TestCase( "1.2.3", "=1.2.3", "1.2.3[Lock]" )]
    [TestCase( ">=1.2.3", ">=1.2.3", "1.2.3" )]
    [TestCase( "^1.2.3", "^1.2.3", "1.2.3[LockMajor,CI]" )]
    [TestCase( "^0.0.3", ">=0.0.3", "0.0.3[LockPatch,CI]" )]
    [TestCase( "~12.16", "~12.16", "12.16.0[LockMinor,CI]" )]
    [TestCase( "^0.2.3", "~0.2.3", "0.2.3[LockMinor,CI]" )]
    [TestCase( "^0.1.93", "~0.1.93", "0.1.93[LockMinor,CI]" )]
    [TestCase( "^12", "^12", "12.0.0[LockMajor,CI]" )]
    [TestCase( "^12.8", "^12.8", "12.8.0[LockMajor,CI]" )]
    [TestCase( "^12.8.1", "^12.8.1", "12.8.1[LockMajor,CI]" )]
    [TestCase( "~0.1", "~0.1", "0.1.0[LockMinor,CI]" )]
    [TestCase( "~0.1.15", "~0.1.15", "0.1.15[LockMinor,CI]" )]
    ...
    public void use_NormalizeNpmVersionBoundAll_to_handle_SVersionBoundAll_with_or_without_includePrerelease_flag( string bound )
    {
        ...
        static void Check( string bound, bool includePrerelease )
        {
            ...
            rNpm.IsValid.Should().BeTrue();
            (rNpm.Result == SVersionBound.All).Should().Be( includePrerelease || bound == ">=0.0.0-0" );
            rNpm.Result.NormalizeNpmVersionBoundAll().Should().Be( SVersionBound.All );
        }
    }
}
